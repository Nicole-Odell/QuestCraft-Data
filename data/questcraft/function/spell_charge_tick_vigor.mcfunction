...
execute if score @s vigor_aspect.charge = @s vigor_aspect.maxCharge unless score @s vigor_aspect.charge > _globals spellCastingMaxCharge run function questcraft:spell_build_vigor
...
execute if score @s vigor_aspect.charge = _globals spellCastingMaxCharge run execute store result storage questcraft:args mageId int 1 run scoreboard players get @s mageId
execute if score @s vigor_aspect.charge = _globals spellCastingMaxCharge run data modify storage questcraft:args castSource set value 0

# Attempt to cast. May or may not succeed based on spell details and cast source
execute if score @s vigor_aspect.charge = _globals spellCastingMaxCharge run function questcraft:spell_casting_prime with storage questcraft:args